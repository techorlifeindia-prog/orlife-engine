...
ssh -i "C:\Project\orlife-connect\oracle-ssh-key.key" -o StrictHostKeyChecking=no ubuntu@129.225.118.77 "cd /app && git pull origin master && npm install && npm run build && pm2 restart all && pm2 status"
