...
export function TemplateCard({ template, onEdit, onDelete }: TemplateCardProps) {
  ...
  return (
    <div className="p-5 rounded-2xl border border-[#163546] bg-[#0c1f2b]/90 backdrop-blur-md shadow-lg hover:border-emerald-500/40 hover:shadow-emerald-500/5 transition-all duration-200 flex flex-col justify-between group">
      <div>
        {/* Card Header */}
        <div className="flex justify-between items-start mb-3.5 gap-2">
          <div>
            ...
            <h3 className="font-bold text-base mt-2 text-slate-100 group-hover:text-emerald-300 transition-colors line-clamp-1">
              {template.title}
            </h3>
          </div>

          {/* Quick Actions */}
          <div className="flex items-center gap-1 bg-[#07151e] p-1 rounded-xl border border-[#183647] shrink-0">
            <button
              onClick={handleCopy}
        ...
        <div className="bg-[#06141c] border border-[#163546] rounded-xl p-3.5 text-xs text-slate-300 font-sans whitespace-pre-wrap leading-relaxed min-h-[92px]">
          {renderFormattedContent(template.content)}
        </div>
      </div>

      {/* Footer */}
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-[#163546]/60 text-xs">
        <span className="text-slate-400 text-[11px] flex items-center gap-1">
          <Clock className="w-3 h-3 text-slate-500" /> Updated: {template.updatedAt}
        </span>

        <button
          onClick={handleUseInCampaign}
          className="bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-3 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all hover:scale-[1.02] active:scale-95 shadow-sm"
        >
