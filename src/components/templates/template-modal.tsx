...
export function TemplateModal({ isOpen, onClose, onSave, editingTemplate }: TemplateModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-[#0b1d28] border border-[#1b3a4e] text-slate-100 rounded-2xl max-w-lg w-full p-6 shadow-2xl relative">
        {/* Modal Header */}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-xs font-semibold text-slate-400 block mb-1.5">
              Template Title <span className="text-emerald-400">*</span>
            </label>
            <input
              type="text"
          ...
          <div>
            <div className="flex justify-between items-center mb-1.5">
              <label className="text-xs font-semibold text-slate-400">
                Message Content <span className="text-emerald-400">*</span>
              </label>
            ...
            <div className="flex flex-wrap gap-1.5 mb-2.5">
              {commonTags.map((tag) => (
                <button
                  ...
                  className="text-[11px] bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-2 py-0.5 rounded-lg font-mono flex items-center gap-1 transition-colors"
                >
            ...
            <textarea
              ...
              className="w-full bg-[#06141c] border border-[#1b3a4e] rounded-xl p-3 text-sm text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 resize-none font-sans leading-relaxed transition-all"
            />
          </div>
