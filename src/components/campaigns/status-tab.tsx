    ...
    <div className="space-y-4 animate-in fade-in duration-200">
      ...
      <div className="glass-card rounded-2xl p-5 shadow-lg flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className={`w-4 h-4 rounded-full shrink-0 ${isDeviceOnline ? "bg-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.8)] animate-pulse" : "bg-slate-400"}`}></div>
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-sm text-slate-900 dark:text-white">{deviceProfileName}</h3>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                isDeviceOnline
                  ? "bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border-emerald-500/30"
                  : "bg-slate-500/20 text-slate-500 border-slate-500/30"
              }`}>
                {isDeviceOnline ? "Active & Connected" : "Offline"}
              </span>
            </div>
            <p className="text-xs text-emerald-600 dark:text-emerald-400 font-mono mt-0.5">📱 {deviceOwnerNumber}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="flex items-center gap-2 bg-slate-50 dark:bg-[#06141c] px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-[#163546]">
            ...
            >
              ...
              <option value="Normal" className="bg-[#0b1e28] text-white">⚡ Normal (7s Recommended)</option>
              <option value="Turbo" className="bg-[#0b1e28] text-white">⚡ Turbo (3s Fast Dispatch)</option>
            </select>
          </div>
        </div>
      </div>
